...
public class Lab1_Graph {
    ...
    static class DirectedGraph {
                ...
                for (String v : vertices) {
                    ...
                    for (String u : vertices) {
                        ...
                        if (edges.containsKey(v)) {
                            ...
                            if (outDeg > 0) {
                                sum += pr.get(u) * edges.get(v) / outDeg; // 注意：权重影响？PageRank通常不考虑权重，但这里边有权重，我们可以按权重比例分配
                                // 但标准PageRank不考虑权重，只考虑链接数。实验未明确，我们按标准处理：每个出边均分PR值，与权重无关。
                                // 为了符合直觉，也可以按权重比例分配。这里我们按标准（所有出边均分），因为实验例子中new的PR值0.24可能与权重无关。
                                // 但题目未要求权重影响，所以我们使用标准方法：每个出边均分PR。
                                // 上面用了权重，下面改为均分：
                                // sum += pr.get(u) / outDeg;
        ...
        // 修正：上面注释提到权重问题，我们按标准PageRank实现（不考虑边的权重，只考虑链接数）
        // 重新实现computePageRank，使每个出边均分PR值
